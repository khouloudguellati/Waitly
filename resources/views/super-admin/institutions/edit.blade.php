...
@section('title', 'Modifier Institution')
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white shadow sm:rounded-lg">
            <div class="px-4 py-5 sm:p-6">
                ...
                <form action="{{ route('super-admin.institutions.update', $institution) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            <label for="name" class="block text-sm font-medium text-gray-700">Nom de l'Institution
                                *</label>
                        ...
                        <div>
                            <label for="type" class="block text-sm font-medium text-gray-700">Type d'Institution
                                *</label>
                            <select name="type" id="type" required
                                ...
                                <option value="mairie" {{ old('type', $institution->type) === 'mairie' ? 'selected' : '' }}>
                                    Mairie</option>
                                <option value="hopital"
                                    {{ old('type', $institution->type) === 'hopital' ? 'selected' : '' }}>Hôpital</option>
                                <option value="bureau_gouvernemental"
                                    {{ old('type', $institution->type) === 'bureau_gouvernemental' ? 'selected' : '' }}>
                                    Bureau Gouvernemental</option>
                                <option value="autre" {{ old('type', $institution->type) === 'autre' ? 'selected' : '' }}>
                                    Autre</option>
                            </select>
                            @error('type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <textarea name="address" id="address" rows="3" required
                                class="mt-1 block w-full border @error('address') border-red-300 @else border-gray-300 @enderror rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('address', $institution->address) }}</textarea>
                            @error('address')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4"
                                class="mt-1 block w-full border @error('description') border-red-300 @else border-gray-300 @enderror rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('description', $institution->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label class="flex items-center">
                                ...
                                <span class="ml-2 text-sm text-gray-700">Institution active</span>
                            </label>
                        </div>
                    </div>
